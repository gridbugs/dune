...
module Cmd = struct
  ...
  let eval_value ?catch:_ ~args { command; _ } = Ok (Climate.Command.eval command args)
  let run ?catch:_  { command; _ } = Ok (Climate.Command.run command)
end
...
module Arg = struct
  ...
  let some ?(none = "") (conv : 'a conv) : 'a option conv =
    ...
    in
    let completion = Option.map Climate.Arg_parser.Completion.some conv.completion
      |> Option.value ~default:Climate.Arg_parser.Completion.file
