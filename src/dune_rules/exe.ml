...
module Linkage = struct
  ...
  let native_or_custom (ocaml : Ocaml_toolchain.t) =
    ...
    | Error _ -> Memo.map ocaml.version ~f:custom
    | Ok _ -> Memo.return native
  ;;
  ...
  let of_user_config
    ...
    let open Memo.O in
    match m with
    | Other { mode = Byte; kind = Js } -> Memo.return js
    | _ ->
      ...
      let+ flags =
        match m with
        | Byte_complete ->
          Memo.return [ Ocaml.Version.custom_or_output_complete_exe version ]
        | Other { kind; _ } ->
          (match kind with
           | C -> Memo.return c_flags
           | Js -> Memo.return []
           | Exe ->
             Memo.return
               (match link_mode with
                | Byte_with_stubs_statically_linked_in ->
                  [ Ocaml.Version.custom_or_output_complete_exe version ]
                | _ -> [])
           | Object -> Memo.return o_flags
           | Plugin ->
             Memo.return
               (match link_mode with
                | Native -> cmxs_flags
           ...
           | Shared_object ->
             ...
             (match link_mode with
              | Native ->
                ...
                let native_c_libraries = Ocaml_config.native_c_libraries ocaml_config in
  ...
  let+ for_exes =
    Memo.parallel_map programs ~f:(fun { Program.name; main_module_name; loc } ->
      ...
      let* cm_files =
        ...
        Cm_files.make ~obj_dir ~modules ~top_sorted_modules ~ext_obj:lib_config.ext_obj ()
      in
      let+ () =
