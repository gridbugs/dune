open! Import
module Solution = Lock_dir.Solution
...
let version_by_package_name local_packages (solution : Solution.t) =
  ...
  in
  let from_lock_dir =
    Package_name.Map.map solution.packages ~f:(fun pkg -> pkg.info.version)
  in
...
let check_for_unnecessary_packges_in_solution
  solution
  all_non_local_dependencies_of_local_packages
  =
  let unneeded_packages_in_solution =
    let locked_transitive_closure_of_local_package_dependencies =
      match
        Solution.transitive_dependency_closure
          solution
          all_non_local_dependencies_of_local_packages
      with
      | Ok x -> x
    ...
    in
    let all_locked_packages = Package_name.Set.of_keys solution.packages in
    Package_name.Set.diff
      all_locked_packages
      locked_transitive_closure_of_local_package_dependencies
  in
  if Package_name.Set.is_empty unneeded_packages_in_solution
  then ()
  else (
    let packages =
      Package_name.Set.to_list unneeded_packages_in_solution
      |> List.map ~f:(Package_name.Map.find_exn solution.packages)
    in
    User_error.raise
      ~hints:lockdir_regenerate_hints
      [ Pp.text
...
let validate t =
  validate_dependency_hash
    t.local_packages
    ~saved_dependency_hash:t.dependency_hash_from_lockdir;
...
let contains_package t package_name =
  ...
  let in_lock_dir = Package_name.Map.mem t.solution.packages package_name in
