...
module Dependency_set : sig
  ...
  (** Returns a hash (SHA512) of all dependencies in the set or
      [`Empty] if the set is empty. The reason for behaving differently when
      the set is empty is so that callers are forced to explicitly handle the
      case where there are no dependencies which will likely lead to better
      user experience. *)
  val hash_or_empty : t -> (Dependency_hash.t, [ `Empty ]) result
