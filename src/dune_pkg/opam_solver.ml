...
let opam_package_to_lock_file_pkg
  ...
  let depends =
    match
      Resolve_opam_formula.filtered_formula_to_package_names
        ...
        opam_file.depends
    with
    | Ok dep_package_names -> Package_name.Set.of_list dep_package_names
  ...
  in
  let depopts = available_depopts solver_env version_by_package_name opam_file in
  let depends =
    Package_name.Set.union depends depopts
    |> Package_name.Set.to_list
    |> List.map ~f:(fun package_name -> Loc.none, package_name)
  in
