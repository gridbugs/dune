...
let dependency_transitive_closure t start =
  let missing_packages =
    let all_packages_in_lock_dir =
      Package_name.Map.keys t.packages |> Package_name.Set.of_list
    in
    ...
    let rec loop seen =
      ...
      | Some node ->
        let unseen_deps =
          ...
          Package_name.Map.keys (Package_name.Map.find_exn t.packages node).deps
          |> List.filter ~f:(Package_name.Set.mem seen)
